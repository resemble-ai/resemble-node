...
const Resemble = Something.Resemble
...
//   const project = await Resemble.v2.projects.create({ name: 'Test Project', description: 'Test Description', is_archived: false, is_collaborative: false, is_public: false })
//   expect(project.success).toEqual(true)
//   const updated_project = await Resemble.v2.projects.update(project.item.uuid, { name: 'Updated Name', description: 'Updated description' })
...
test('clips', async () => {
  ...
  // const syncClip = await Resemble.v2.clips.createSync(projectUuid, { voice_uuid: getTestVoiceUUID(), body: 'This is a test', is_archived: false, is_public: false })
  ...
  try {
    const streamedClipResponse = []
    for await (const buff of Resemble.v2.clips.stream({ 
      data: 'This is a test',
      project_uuid: projectUuid,
      voice_uuid: getTestVoiceUUID() 
    })) {
      expect(buff).toBeTruthy()
      streamedClipResponse.push(buff)
    }
  } catch (e) {
    fail('streaming returned error')
  }
...
//   const recording = await Resemble.v2.recordings.create(voiceUuid, { name: 'Test recording', text: 'transcription', is_active: true, emotion: 'neutral' }, stream, size)
//   expect(recording.success).toEqual(true)
//   const updatedRecording = await Resemble.v2.recordings.update(voiceUuid, recording.item.uuid, {name: 'New name', text: 'new transcription', is_active: true, emotion: 'neutral'})
//   expect(updatedRecording.success).toEqual(true)
//   const fetchedRecording = await Resemble.v2.recordings.get(voiceUuid, recording.item.uuid)
//   expect(fetchedRecording.success).toEqual(true)
//   const deleteOp = await Resemble.v2.recordings.delete(voiceUuid, recording.item.uuid)
//   expect(deleteOp.success).toEqual(true)
// })